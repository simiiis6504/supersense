    ...
    <div ref={ref} className="relative inline-flex"
      ...
      {visible && rect && (
        <div className="fixed z-[9999] pointer-events-none"
          ...
          <div className="bg-zinc-900 border border-zinc-700/60 rounded-xl px-3 py-2 text-[10px] text-zinc-300 leading-relaxed shadow-2xl"
  ...
  return (
    <div className="relative inline-flex">
      <button ref={ref} onClick={handleClick}
        className="w-3.5 h-3.5 rounded-full border border-zinc-600 text-zinc-600 hover:border-zinc-400 hover:text-zinc-400 transition-colors flex items-center justify-center text-[8px] font-black leading-none">
        i
      </button>
      {open && (
        <>
          ...
          {rect && (
            <div className="fixed z-50 bg-zinc-900 border border-zinc-700/60 rounded-2xl p-3 shadow-2xl"
              style={{ width: popWidth, left: leftPos, top: topPos, transform: 'translateY(-100%)' }}>
...
const GLOSSARY: Record<string, string> = {
  ATL: 'Acute Training Load — your average training stress over the last 7 days. High ATL = high recent fatigue. Calculated from workout calories.',
  CTL: 'Chronic Training Load — your average training stress over the last 42 days. This represents your fitness base. Higher CTL = more fitness.',
  TSB: 'Training Stress Balance = CTL − ATL. Positive = fresh/tapered (fitness > recent fatigue). Negative = fatigued/building. Ideal race TSB: +5 to +25.',
  RHR: 'Resting Heart Rate — measured during sleep. Lower is generally better for aerobic athletes. Elevated RHR often signals fatigue or illness.',
  HRV: 'Heart Rate Variability — the variation in time between heartbeats (in ms). Higher = better recovery and autonomic nervous system health. Estimated from sleep quality and RHR.',
  PAI: 'Personal Activity Intelligence — a weekly score from Zepp. Maintaining 100+ PAI is linked to reduced cardiovascular risk in research studies.',
  ...
  Strain: 'Daily training load from 0–21. Based on workout intensity × duration plus active steps. Analogous to Whoop strain.',
  SpO2: 'Blood oxygen saturation. Normal: 95–100%. Below 94% warrants monitoring. Can dip slightly at altitude or with respiratory illness.',
  TDEE: 'Total Daily Energy Expenditure — estimated calories burned. BMR + NEAT (steps) + Exercise + Thermic Effect of Food.',
  NEAT: 'Non-Exercise Activity Thermogenesis — calories burned through daily movement (steps, fidgeting, posture). Often underrated for body composition.',
  TSB_detail: 'TSB > +15: Peak form, great for racing or testing. TSB 0 to +15: Good for quality workouts. TSB -10 to 0: Building/productive. TSB < -20: Overreaching, consider rest.',
};
...
const WORKOUT_ICONS: Record<string, string> = {
  ...
  'Outdoor Cycling': '🚴', 'Indoor Cycling': '🚴', 'Strength Training': '🏋️',
  'Stretching': '🧘', 'HIIT': '⚡', 'Outdoor Hiking': '🥾',
  'Jump Rope': '🪢', 'Elliptical': '🔄', 'Open Water Swimming': '🏊',
  'Free Training': '💪', 'Yoga': '🧘', 'Disco': '🕺', 'Dance': '💃',
  'Ski': '⛷️', 'Paddleboarding': '🏄', 'Skateboard': '🛹',
  'Street Dance': '💃', 'Core Training': '🔥', 'Stair Stepper': '🪜',
  'Floor Climbing Machine': '🧗', 'Group Calisthenics': '🤸',
};
...
const isRunning = (w: Workout) =>
  ['Outdoor Running', 'Treadmill', 'Race Walking'].includes(getWorkoutName(w));
...
function computeMetrics(data: DailyHealth[], workouts: Workout[]) {
  ...
  const last7WorkoutCal = workouts.filter(w => { const d = new Date(); d.setDate(d.getDate() - 7); return new Date(w.date) >= d; }).reduce((s, w) => s + (w.calories || 0), 0);
  const last42WorkoutCal = workouts.filter(w => { const d = new Date(); d.setDate(d.getDate() - 42); return new Date(w.date) >= d; }).reduce((s, w) => s + (w.calories || 0), 0);
  const atl = Math.round(last7WorkoutCal / 7);
  const ctl = Math.round(last42WorkoutCal / 42);
  const tsb = ctl - atl; // TSB = CTL − ATL (positive = fresh, negative = fatigued)
  ...
  const avgSleep7 = parseFloat((last7.reduce((s, d) => {
    const ds = d.sleep_total_minutes || ((d.sleep_deep_minutes || d.deep_sleep_minutes || 0) + (d.sleep_rem_minutes || d.rem_sleep_minutes || 0) + (d.sleep_light_minutes || d.shallow_sleep_minutes || 0));
    return s + ds;
  }, 0) / last7.length / 60).toFixed(1));
  ...
  const chartData = last30.map((d, i) => {
    ...
    const dTotal = d.sleep_total_minutes || (dDeep + dRem + dLight) || 0;
    return {
      date: i % 5 === 0 ? d.date.slice(5) : '',
      fullDate: d.date,
      RHR: d.hr_resting || 0,
      Steps: d.steps || 0,
      StepsK: Math.round((d.steps || 0) / 100),
      Sleep: parseFloat((dTotal / 60).toFixed(1)),
      Score: d.sleep_score || 0,
      Stress: d.stress_current || 0,
  ...
  if (tsb < -20) insights.push(`High fatigue accumulated (TSB: ${tsb}). CTL below recent load — deload week recommended.`);
  else if (tsb > 15) insights.push(`Fresh & tapered (TSB: +${tsb}). Fitness exceeds recent fatigue — peak performance window.`);

  return {
    latest, sorted, last7, last30, chartData,
    recoveryScore, strainScore, estHRV, atl, ctl, tsb,
...
function InteractiveBarChart({ data, dataKey, color, height = 64, unit = '', label = '' }: {
  data: any[]; dataKey: string; color: string; height?: number; unit?: string; label?: string;
}) {
  ...
  const tooltipLeft = hovered ? Math.min(Math.max(hovered.rect.left + hovered.rect.width / 2 - tooltipWidth / 2, 8), window.innerWidth - tooltipWidth - 8) : 0;
  const tooltipTop = hovered ? hovered.rect.top - 8 : 0;

  const displayVal = hovered !== null ? vals[hovered.i] : null;
      ...
      <div className="flex items-end gap-px w-full" style={{ height }}>
        {vals.map((v, i) => (
          <div key={i} className="relative flex-1 flex flex-col items-center justify-end"
            ...
            <div className="w-full rounded-t-[2px] transition-all duration-200"
              style={{
                height: `${Math.max(2, (v / max) * height)}px`,
                backgroundColor: color,
                opacity: hovered === null ? (0.4 + 0.6 * (v / max)) : hovered.i === i ? 1 : 0.2,
                transform: hovered?.i === i ? 'scaleY(1.03)' : 'scaleY(1)',
                transformOrigin: 'bottom',
              }} />
          </div>
        ))}
      </div>
      {hovered !== null && displayVal !== null && displayVal > 0 && (
        <div className="fixed z-[9999] pointer-events-none text-center"
          style={{ left: tooltipLeft, top: tooltipTop, transform: 'translateY(-100%)', width: tooltipWidth }}>
          <div className="bg-zinc-900 border border-zinc-700 rounded-lg px-2 py-1.5 shadow-xl inline-block">
            ...
            <div className="text-[11px] font-black" style={{ color }}>
              {dataKey === 'Steps' ? `${(displayVal / 1000).toFixed(1)}k` : displayVal}{dataKey !== 'Steps' ? unit : ''}
            </div>
          </div>
        </div>
      )}
...
function InteractiveSparkLine({ data, dataKey, color, height = 56, fill = false, unit = '' }: {
  data: any[]; dataKey: string; color: string; height?: number; fill?: boolean; unit?: string;
}) {
  const [hovered, setHovered] = useState<{ i: number; rect: DOMRect } | null>(null);
  const vals = data.map(d => Number(d[dataKey]) || 0);
  const max = Math.max(...vals, 1);
  ...
  const pts = vals.map((v, i) =>
    `${(i / Math.max(vals.length - 1, 1)) * W},${v > 0 ? H - pad - ((v - min) / range) * (H - pad * 2) : H - pad}`
  ).join(' ');

  const getX = (i: number) => (i / Math.max(vals.length - 1, 1)) * W;
  const getY = (v: number) => v > 0 ? H - pad - ((v - min) / range) * (H - pad * 2) : H - pad;
    ...
    <div className="relative w-full" style={{ height }}>
      <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" className="w-full absolute inset-0" style={{ height }}>
        ...
        {hovered !== null && vals[hovered.i] > 0 && (
          <>
            <line x1={getX(hovered.i)} y1={0} x2={getX(hovered.i)} y2={H} stroke={color} strokeWidth="0.8" strokeDasharray="2,2" opacity="0.4" />
            <circle cx={getX(hovered.i)} cy={getY(vals[hovered.i])} r="3" fill={color} />
          </>
        )}
      </svg>
      ...
      <div className="flex flex-wrap gap-[2px]">
        {data.map(([, hr], i) => {
          ...
          return <div key={i} onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(null)}
            style={{ width: 6, height: 12, borderRadius: 1, backgroundColor: color, transform: hovered === i ? 'scaleY(1.5)' : 'scaleY(1)', transition: 'transform 0.1s', cursor: 'crosshair' }} />;
        })}
      </div>
    </div>
  );
...
function SleepBar({ deep, rem, light, wake }: { deep: number; rem: number; light: number; wake: number }) {
  ...
  return (
    <div className="relative">
      {hovered && (
        <div className="absolute -top-8 left-0 bg-zinc-900 border border-zinc-700 rounded-lg px-2 py-1 text-[10px] font-black z-10 pointer-events-none text-zinc-200">
          {segs.find(s => s.label === hovered)?.desc}
        </div>
      )}
      <div className="flex h-4 rounded-full overflow-hidden gap-[1px]">
        {segs.map((s, i) => (
          <div key={i} style={{ flex: s.v, backgroundColor: s.color, cursor: 'pointer', opacity: hovered && hovered !== s.label ? 0.4 : 1, transition: 'opacity 0.15s' }}
...
// ─── UI ATOMS ─────────────────────────────────────────────────────────────────
function MetricCard({ label, value, unit, color, sub, icon, onClick, tooltip }: {
  label: string; value: any; unit?: string; color?: string; sub?: string; icon?: string; onClick?: () => void; tooltip?: string;
}) {
  return (
    <div onClick={onClick}
      className={`bg-zinc-900/60 border border-zinc-800/50 rounded-2xl p-4 flex flex-col gap-2 hover:border-zinc-700/60 transition-all ${onClick ? 'cursor-pointer hover:bg-zinc-900/80' : ''}`}>
      ...
      <div className="flex items-baseline gap-1 mt-auto">
        <span className="text-[1.6rem] font-black leading-none tracking-tight" style={{ color: color || '#f1f5f9' }}>
          {value ?? '--'}
        </span>
...
function ScoreRing({ value, max, color, label, sub, size = 110, glossaryKey }: {
  value: number; max: number; color: string; label: string; sub?: string; size?: number; glossaryKey?: string;
}) {
  return (
  ...
  return (
    <button onClick={onClick} className="w-full flex items-center gap-4 px-4 py-3.5 rounded-xl hover:bg-zinc-800/40 transition-all text-left group">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-black text-zinc-100 uppercase tracking-tight truncate">{name}</p>
      ...
      <div className="relative w-full md:max-w-2xl max-h-[90vh] overflow-y-auto bg-[#0d0d0f] border border-zinc-800/60 rounded-t-3xl md:rounded-3xl">
        <div className="sticky top-0 bg-[#0d0d0f]/95 backdrop-blur border-b border-zinc-800/40 px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-black text-zinc-100 uppercase tracking-tight">{name}</h2>
        ...
        <div className="p-6 space-y-6">
          ...
          {hrZones && (
            <div>
              <h3 className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500 mb-3">HR Zone Distribution</h3>
              <div className="space-y-2">
                {[{ label: 'Z1 Recovery', color: '#34d399' }, { label: 'Z2 Aerobic', color: '#60a5fa' }, { label: 'Z3 Tempo', color: '#fbbf24' }, { label: 'Z4 Threshold', color: '#fb923c' }, { label: 'Z5 Anaerobic', color: '#f43f5e' }].map((z, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <span className="text-[9px] font-black uppercase text-zinc-500 w-24 shrink-0">{z.label}</span>
                    <div className="flex-1 h-2 bg-zinc-800 rounded-full overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-700" style={{ width: `${hrZones[i]}%`, backgroundColor: z.color }} />
                    </div>
                    <span className="text-[10px] font-black text-zinc-300 w-8 text-right">{hrZones[i]}%</span>
                  </div>
                ))}
              </div>
            </div>
...
export default function SuperSenseDashboard() {
  ...
  const loadData = useCallback(async () => {
    ...
    const normalizedSleep = (sleepRes.data || []).map((s: any) => ({
      ...
      sleep_light_minutes: s.sleep_light_minutes || s.shallow_sleep_minutes || 0,
  ...
  const runAI = async (m: NonNullable<ReturnType<typeof computeMetrics>>) => {
    ...
    try {
      ...
      const res = await fetch('/api/ai', {
        ...
        body: JSON.stringify({ prompt: initialPrompt, system: systemPrompt })
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      const reply = data.text || m.insight;
      setAiMessage(reply);
      setChatHistory([{ role: 'user', content: initialPrompt }, { role: 'assistant', content: reply }]);
  ...
  const sendChatMessage = async (userMsg: string, currentMetrics: NonNullable<ReturnType<typeof computeMetrics>>) => {
    ...
    setAiLoading(true);
    try {
      const systemPrompt = buildSystemPrompt(currentMetrics);
      const res = await fetch('/api/ai', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt: userMsg, system: systemPrompt, history: chatHistory })
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
    ...
    <div className="min-h-screen bg-[#080809] text-zinc-100" style={{ fontFamily: "'DM Sans', 'Figtree', sans-serif" }}>
      ...
      <div className="relative max-w-[1400px] mx-auto px-4 md:px-8 py-8 space-y-6">
        ...
        {chatOpen && (
          <div className="bg-zinc-900/40 border border-zinc-800/40 rounded-2xl overflow-hidden">
            <div className="flex items-center gap-3 px-5 py-3 border-b border-zinc-800/40">
              <div className="w-5 h-5 rounded-full bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 text-xs shrink-0">🧠</div>
            ...
            <div className="max-h-[420px] overflow-y-auto px-5 py-4 space-y-4">
              {chatHistory.length === 0 && !aiLoading && (
                <div className="text-zinc-600 text-sm text-center py-6">
                  ...
                  <p className="font-bold">Ask your AI coach anything</p>
                  <p className="text-[10px] mt-1">It knows all your biometric data and remembers this conversation</p>
                </div>
              )}
              ...
              {aiLoading && (
                <div className="flex justify-start">
                  <div className="bg-zinc-800/60 border border-zinc-700/30 rounded-2xl rounded-tl-sm px-4 py-3">
          ...
          <div className="flex gap-0 border-b border-zinc-800/40 overflow-x-auto">
            {TABS.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`px-4 py-3 text-[9px] font-black uppercase tracking-[0.2em] whitespace-nowrap border-b-2 -mb-px transition-all flex items-center gap-2 ${tab === t.id ? 'text-emerald-400 border-emerald-400' : 'text-zinc-600 border-transparent hover:text-zinc-400'}`}>
                {t.label}
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                  ...
                  <div className="bg-zinc-900/40 border border-zinc-800/40 rounded-2xl p-5">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <InfoBtn text="Based on workout calories. ATL = last 7 days avg. CTL = last 42 days avg. TSB = ATL − CTL. Positive TSB = fresh, negative = fatigued/building." />
                    </div>
                    <div className="grid grid-cols-3 gap-3 mb-4">
                      {[
                        { v: atl, label: 'ATL', sub: 'Fatigue (7d)', color: '#fb923c', glossary: 'ATL' },
                        { v: ctl, label: 'CTL', sub: 'Fitness (42d)', color: '#10b981', glossary: 'CTL' },
                        { v: tsb, label: 'TSB', sub: 'Form', color: tsb >= 0 ? '#38bdf8' : '#f43f5e', glossary: 'TSB' },
                      ].map(s => (
                ...
                <div className="bg-zinc-900/40 border border-zinc-800/40 rounded-2xl p-5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {[
                      { key: 'RHR' as const, label: 'Resting HR', color: '#f43f5e', unit: 'bpm' },
                      { key: 'Score' as const, label: 'Sleep Score', color: '#c084fc', unit: '/100' },
                      { key: 'Stress' as const, label: 'Stress Level', color: '#fbbf24', unit: '' },
                    ].map(s => {
                ...
                {racePredictions && (
                  <div className="bg-zinc-900/40 border border-zinc-800/40 rounded-2xl p-5">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      {[
                        { dist: '5 KM', ...racePredictions.fiveK, color: '#34d399' },
                        { dist: '10 KM', ...racePredictions.tenK, color: '#38bdf8' },
                        { dist: 'Half', ...racePredictions.half, color: '#818cf8' },
                        { dist: 'Marathon', ...racePredictions.full, color: '#fb923c' },
                      ].map((r, i) => (
        ...
        <footer className="border-t border-zinc-800/20 pt-4 flex justify-between text-[8px] text-zinc-700 font-bold uppercase tracking-widest">
          <span>SuperSense v5 · Amazfit Balance</span>
